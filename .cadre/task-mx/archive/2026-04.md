# Archive — 2026-04

_Archived DONE/SUPERSEDED entries. Append-only. Owned by task-mx-cadre._

## 1. Populate CLAUDE.md workflow section

---
id: 1
title: Populate CLAUDE.md workflow section
status: DONE
priority: null
due: null
scheduled: null
blockers: []
tags: []
project: cadre
impact: null
effort: null
created: null
updated: 2026-04-27
closed: 2026-04-22
---

**Status: DONE (2026-04-22).** Workflow section appended to CLAUDE.md with L2 (grounded) citations. Three subsections landed: Git and review architecture, Agent architecture, Working with me. Sourcing drawn from isolated research brief at `.cadre/citations/2026-04-22-workflow.md`. Five items marked Cadre-original (sentence/page/chapter ladder, base-3 recursive retro, emerge-as-needed `.cadre/` policy, 0-10 yanking scale, assistant-maintained tab stack). Unverified 2026 citations passed through without live verification — audit-later candidates noted in handoff.

---

Encode the decisions made during the bootstrap walkthrough into CLAUDE.md.

**Workflow rules (concise):**
- Worktree convention: sibling directory `C:\Users\duham\Projects\cadre.worktrees\<branch>`; per feature/task, reused across sessions until the branch merges.
- `.git/info/exclude` is the right place for personal ignores (not `.gitignore`).
- Plan mode precedes every non-trivial execution session; approval gates implementation.
- Conventional Commits enforced by commit-msg hook (once installed — TODO #21).
- When unsure whether to commit or keep going, commit.
- Three-review vocabulary: commit review / push review / merge review; book metaphor (sentence / page / chapter); hot/cold vocabulary retired.
- Retros end-of-session, non-failure-gated.
- **Defer and TBD are achievements, not failures.** The *level of resolution* matters as much as resolution. If the right answer is "we don't know yet" or "this is blocked on X," capture that and move on — don't force premature closure.

**Iron law: every agent has (input schema) → (output artifact).** Structural requirement for every agent in Cadre:
- Declared input schema (what it reads/receives).
- Declared output artifact (a structured, durable file or piece thereof).
- Declared file footprint (which paths it reads and writes). Outside that = bug.

Applies to every agent in the project. Every agent-building TODO must include explicit input/output schemas.

**File hygiene (first-class architectural concern).** Third attempt at this harness; prior attempts blew up partly on file-hygiene decay. Three-tier namespace:
- Root: `CLAUDE.md`, `HANDOFF.md`, `TODOS.md` (human-facing), plus `.env` / `.gitignore` / `.cadre/`.
- `.cadre/` = Cadre infrastructure namespace (contents emerge — no prescribed taxonomy).
- Product source: its own namespace, TBD at stack choice.

**Disciplines:**
1. Namespace boundary — root holds exactly three human-facing files plus `.env` / `.gitignore` / `.cadre/`. No infra files at root.
2. `.cadre/` contents emerge — subfolders created at moment of need. No anticipation folders.
3. Modularity in agent specs, not folder layout — agent file footprint declares read/write paths.
4. Re-organize reactively — only when patterns demonstrably emerge does a grouping earn its place.
5. Naming — deterministic, greppable.
6. Secrets — exactly `.env` / `.env.*`. Nothing else holds secrets. `.gitignore` stays narrow.
7. Lifecycle — every artifact type has a documented retention policy at creation (even if "keep forever").
8. Agent file footprint declares read/write paths. Outside = bug.
9. Pruning is first-class — owned by an agent or scheduled task, never manual cleanup.

---

## 12. Build the handoff maintainer agent

---
id: 12
title: Build the handoff maintainer agent
status: DONE
priority: null
due: null
scheduled: null
blockers: []
tags: []
project: cadre
impact: null
effort: null
created: null
updated: 2026-04-27
closed: 2026-04-26
---

**Status: DONE (2026-04-26).** Shipped as `handoff-mx-cadre` — boundary-synthesizer architecture (ADR-072). Logger hook appends events to `.cadre/session-events.log` non-blocking; synthesizer subagent dispatched by SessionStart (matchers: `startup|resume|clear|compact`) and SessionEnd integrates events into a four-section entry (Narrative / Decisions / Active Items / Changes). One active entry at `.cadre/handoff.md`; prior entries archived to `.cadre/handoffs/<ISO-date>.md`. Surface hook (SessionStart Action 2) injects the active entry verbatim via `additionalContext`. Plan: `.cadre/plans/ok-let-s-finally-start-pure-dusk.md`. Files shipped: `.claude/agents/handoff-mx-cadre.md`, `.claude/hooks/handoff-mx-{logger,surface}-cadre.ts`, `.claude/settings.json`, `.gitignore` updates.

Original contract (preserved as history):

Runs fully autonomous in the background. Contract:
- **Live mutation during session** — entry is updated continuously as in-session actions occur (not just at session end).
- **Session-end stamp** — the live entry is sealed into the final record at session end.
- **Session-start surfacing** — relevant prior handoff content is surfaced into new sessions automatically.
- **Lifecycle/archive** — handles aging of old entries (no manual prune; archive, don't delete).
- **User never has to prompt it.** No "update the handoff" or "read the handoff" requests should ever be needed.
- Per-commit narrative lives in commit messages (via the commit-review gate), not in handoff. Handoff is session-level.

Explicit (input schema) → (output artifact) contract required per the iron law.

---

## 16. CLAUDE.md addition for premature-bounding / response-bloat failure modes

---
id: 16
title: CLAUDE.md addition for premature-bounding / response-bloat failure modes
status: DONE
priority: null
due: null
scheduled: null
blockers: []
tags: []
project: cadre
impact: null
effort: null
created: null
updated: 2026-04-27
closed: 2026-04-22
---

**Status: DONE (2026-04-22).** Absorbed into TODO #1's workflow-section writeout. "Corrections stay at earned scope," "Exchange rhythm," and "Don't pre-invent taxonomy" items in the Working-with-me subsection cover the premature-bounding and response-bloat failure modes as a temp hack ahead of the engram revival (TODO #15).

---

Temp hack ahead of the engram revival (TODO #15). Content and placement TBD.

---

## 27. The Playbook — canonical-state mirroring across worktrees

---
id: 27
title: The Playbook — canonical-state mirroring across worktrees
status: SUPERSEDED
priority: null
due: null
scheduled: null
blockers: []
tags: []
project: cadre
impact: null
effort: null
created: null
updated: 2026-04-27
closed: 2026-04-26
---

**Status: DONE — superseded (2026-04-26).** Initial design (bidirectional sync hook + JSON config + worktree-init script) committed on `feat/playbook-cadre` (commit `1345135`) but never merged. Architecture pivoted via plan-cadre to **plain tracking** of `.cadre/` and `CLAUDE.md` (PR #4, merged as `8570e36`); CC native worktree primitives (`claude --worktree`) replace the custom shell wrapper. The "playbook" concept itself dissolves — there is no playbook in the new architecture. Full reasoning in `.cadre/plans/what-di-you-think-floofy-rose.md` and ADRs 063 / 064 / 065 / 067.

---

## 28. `.claude/` Reddit-style restructure (deferred from PR #4)

---
id: 28
title: "`.claude/` Reddit-style restructure (deferred from PR #4)"
status: DONE
priority: null
due: null
scheduled: null
blockers: []
tags: []
project: cadre
impact: null
effort: null
created: null
updated: 2026-04-27
closed: 2026-04-26
---

**Status: DONE — scaffold-only (2026-04-26).** Empty `.claude/{rules, hooks, commands, agents}/` directories scaffolded with `.gitkeep`; `skills/` and `worktrees/` already existed. CLAUDE.md doctrine entry "**`.claude/` directory shape**" added under "Git and review architecture" describing the layout, the `*.local.md` / `*.local.json` personal-config convention (added to `.gitignore`), and the "shape without substance" stance — subdirs sit empty until real usage warrants a primitive. Stale empty `.claude/references/` (leftover from prior session's move to `.cadre/references/`) removed. **Out of scope (deferred):** content migration from CLAUDE.md to `rules/` (no specialized guidance feels crowded yet); namespace cleanup `.cadre/references/` → `.cadre/references/` (different category — design refs vs operational state — so leaving distinct). Both available as future work when felt-need surfaces. ADR-068.

---

## 3. Build the taskboard runner agent that owns TODOS.md

---
id: 3
title: Build the taskboard runner agent that owns TODOS.md
status: DONE
priority: H
due: null
scheduled: null
blockers: []
tags: [infra, mx-agent]
project: cadre
impact: 3
effort: 3
created: 2026-04-22
updated: 2026-04-27
closed: 2026-04-27
---

**Status: DONE (2026-04-27).** Shipped as `task-mx-cadre` — single-writer + deterministic-script split per ADR-074. Architecture: orchestrator writes inbox shards; agent is the SOLE writer of `.cadre/todos.md`. Taskwarrior polynomial urgency scoring with config at `.cadre/task-mx/weights.json`. Three-column Index (Ready / Blocked / In Progress) regenerated each fire by `task-mx-cadre.score.ts`. Kahn topological sort with cycle detection; refuse-and-emit on cycle (rejected shards land in `.cadre/task-mx/inbox/.rejected/`). Day-1 migration of 31 legacy entries completed with dry-run user-gate; 5 DONE archived to `.cadre/task-mx/archive/2026-04.md`. Three-hook ecosystem: trigger (PostToolUse on inbox writes), surface (UserPromptSubmit, 3s debounce floor), no prime (deferred until felt). PR #22 merged to main as commit `f0f101a`.

Original contract (preserved as history):

First infrastructural agent, because every other agent's work routes *through* the task board. Build order: this → handoff maintainer (TODO #12) → the rest.

Input: task-board file, user/agent updates (via inbox shards). Output: maintained TODOS.md with regenerated Index. Iron law I/O contract declared in `.claude/agents/task-mx-cadre.md`.

---
## 32. Smoke test — task-mx skill E2E pipeline verification

---
id: 32
title: Smoke test — task-mx skill E2E pipeline verification
status: DONE
priority: L
due: null
scheduled: null
blockers: []
tags: [smoke-test, task-mx]
project: cadre
impact: 1
effort: 1
created: 2026-04-28
updated: 2026-04-28
closed: 2026-04-28
---

**What:** End-to-end verification that the new orchestrator-side `task-mx-cadre` skill plus `score.ts --integrate` pipeline lands a new entry on the board correctly.

**Why:** First exercise of the simplified pipeline (subagent and hooks retired). Confirms that a shard dropped via the orchestrator → script flow integrates cleanly: appears in Index Ready, lands in Detail with full frontmatter + body, gets a generated urgency comment, and the consumed shard is deleted from inbox.

**Acceptance:**
- Entry visible in `## Ready` table with non-zero urgency ✓
- Entry present in `# Detail` with full 14-field frontmatter and prose body ✓
- Inbox shard deleted ✓
- Subsequent close-shard archives this entry to `.cadre/task-mx/archive/2026-04.md` and removes it from `todos.md`

**Refs:** Tracks alongside the four-step migration that retired `.claude/hooks/task-mx-{trigger,surface}-cadre.ts` and `.claude/agents/task-mx-cadre.{md,refs.md}`. Score script at `.claude/agents/task-mx-cadre.score.ts`. Skill at `.claude/skills/task-mx-cadre/SKILL.md`.

**Closed (2026-04-28):** smoke test passed; archive flow verified.
## 22. 

---
id: 22
title: 
status: DONE
priority: null
due: null
scheduled: null
blockers: []
tags: []
project: null
impact: null
effort: null
created: null
updated: 2026-04-29
closed: 2026-04-29
---

Closed: PRs #26 (CI bootstrap, ae4ad0b) and #28 (first test, 3326416) merged to main; branch protection enabled on main requiring ci-complete. All 8 deterministic checks live and gating PRs (typecheck, oxlint, knip, fallow, comment-ratio, check-todos, check-loc, check-test-exists) plus workflow-lint (actionlint) and ci-complete aggregator. Plan artifacts at `.claude/plans/base-ci-deterministic-stack.md` (Diamond 1) and `.cadre/plans/base-ci-deterministic-stack.md` (Diamond 2).
## 6. 

---
id: 6
title: 
status: DONE
priority: null
due: null
scheduled: null
blockers: []
tags: []
project: null
impact: null
effort: null
created: null
updated: 2026-04-29
closed: 2026-04-29
---

Closed: subsumed by #22. The base CI workflow (PR #26) ships `bunx tsc --noEmit` as the dedicated `typecheck` job; tsconfig.json (strict, moduleResolution: bundler, noEmit, incremental) gates every PR via the `ci-complete` aggregator. No standalone work remains.

## 23. 

---
id: 23
title: 
status: DONE
priority: null
due: null
scheduled: null
blockers: []
tags: []
project: null
impact: null
effort: null
created: null
updated: 2026-04-29
closed: 2026-04-29
---

Closed: branch protection rule applied via `gh api -X PUT repos/WolfOnWings/cadre/branches/main/protection` after #22 landed. Requires `ci-complete` status check; strict=true; enforce_admins=false. Configured 2026-04-29.

