...
**Default flow: branch + PR.** `git checkout -b <name>`, edit, commit, push, PR, merge. The three-review gates (commit / push / merge) fire on git operations — branches alone are the full workflow for sequential single-session work, which is most cases. Personal ignores live in `.git/info/exclude`, shared ignores in `.gitignore`. **Operational-metadata exception:** trivial mutations to `.cadre/todos.md`, `.cadre/handoff.md`, or the ADR log can land directly on main — branch+PR ceremony cost outweighs review value when there's no code, behavior, or doctrine change.
...
**Per-task commits in multi-task work.** When the work is a sprint, todo list, or multi-step plan, the natural division is per-task — commit each task as its own commit before starting the next. Bundling at the end shares a goal but defeats `git bisect` (Bisect Manual, git-scm.com) and inflates the review unit past what fits the page-sizing metaphor. Narrative across commits belongs in the PR description; commit granularity is the bisect surface.
...
**Creative drilling.** Walk decision trees depth-first with backtracking (Russell and Norvig, *AIMA*; CLRS; LLM-reasoning analogue in Tree of Thoughts, Yao et al., NeurIPS 2023). Silently enumerate major branches at the start of any decision-tree walk, pick the highest-priority branch, walk leaf questions until it resolves, then backtrack to the most recent unresolved juncture. Follow user-opened asides as their own branches; the depth-first stack handles return naturally — do not force back to the parent prematurely.

**Don't redefine CC primitives.** When the user invokes a slash command or names a primitive not in your skills list — especially paired with phrases like "try something new" or "use the X feature" — verify it via `claude-code-guide` before acting. Built-in Claude Code primitives (`/fork`, `/branch`, `/clear`, `/resume`, etc.) have documented semantics and are not yours to redefine to fit available tooling. Improvising an interpretation that "happens to fit" the closest primitive in your toolset is the failure mode — pattern-match on plausibility is how anchoring (Tversky and Kahneman, *Judgment under Uncertainty: Heuristics and Biases*, 1974) bites in tool-selection. Default: verify, don't improvise.
