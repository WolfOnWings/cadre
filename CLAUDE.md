...
**The playbook.** `cadre/playbook.json` lists the gitignored operational-state files that must be present in every active worktree (CLAUDE.md, handoff, todos, ADR log). Two consumers: a worktree-init script (`bun run .claude/hooks/worktree-init-cadre.ts <target>`) populates a fresh worktree from main at creation; a PostToolUse hook (`.claude/hooks/playbook-sync-cadre.ts`, matcher `Edit|Write|MultiEdit`) auto-mirrors any edit to a playbook file across all active worktrees + main, keeping copies byte-identical without manual sync. Edits land where the orchestrator is — local — and the hook propagates. Playbook list is editable in one place (`cadre/playbook.json`). Single canonical state, no merge-back step.
